...
def filter_creation_butter_low_pass_dyn_emg(order=4, freq=3):
    fn = 1000
    [a_low_pass_pos, b_low_pass_pos] = signal.butter(order, freq / fn, 'low')
    return a_low_pass_pos, b_low_pass_pos


def filter_implementation_butter_low_pass_dyn_emg(signal_to_filter, order=4, freq=3):
    a_low_pass_pos, b_low_pass_pos = filter_creation_butter_low_pass_dyn_emg(order, freq)
